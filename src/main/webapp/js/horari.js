...
/**
 * Un cop carregat el DOM, assignem resultats i detall a les variables declarades.
 */
$().ready(() => {
    resultats = $("#results");
    detall = $('#detail');
...
function assignDateToButtons() {
    ...
    const DIA_ACTUAL = DIA_SETMANA * DIA_EN_MILISEGONS;
    let diumenge = new Date(DATA_ACTUAL - (DIA_ACTUAL));

    //Afegim els atributs de dia a cada botó i n'actualitzem l'etiqueta.
    if (DIA_ACTUAL != 0) {//El diumenge obtingut correspon al de la setmana passada, haurem de sumar 1 dia al diumenge d'aquesta setmana
        ...
        //setmana vigent.
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Monday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Monday').html("DL " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Tuesday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Tuesday').html("DT " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Wednesday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Wednesday').html("DC " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Thursday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Thursday').html("DJ " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Friday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Friday').html("DV " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Saturday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Saturday').html("DS " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) + DIA_EN_MILISEGONS);

        $('.Sunday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Sunday').html("DG " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
    } else {//Avui mateix és diumenge. Cal fer que avui sigui el valor del dia de diumenge. Els dies de la setmana seran anteriors a aquest diumenge
        //Traiem 24h entre dia i dia. Com que avui és diumenge, caldrà que la data actual sigui el valor del botó del diumenge
        $('.Sunday').attr('dia', diumenge.toISOString().split("T")[0]);
        $('.Sunday').html("DG " + diumenge.getDate() + "/" + (diumenge.getMonth() + 1));
        diumenge.setTime(Date.parse(diumenge) - DIA_EN_MILISEGONS);
...
function assignBehaviourToDayButtons() {
    $('.Monday').click(() => {
        showActivitiesOnDate($('.Monday').attr('dia'))
    });
    $('.Tuesday').click(() => {
        showActivitiesOnDate($('.Tuesday').attr('dia'))
    });
    $('.Wednesday').click(() => {
        showActivitiesOnDate($('.Wednesday').attr('dia'))
    });
    $('.Thursday').click(() => {
        showActivitiesOnDate($('.Thursday').attr('dia'))
    });
    $('.Friday').click(() => {
        showActivitiesOnDate($('.Friday').attr('dia'))
    });
    $('.Saturday').click(() => {
        showActivitiesOnDate($('.Saturday').attr('dia'))
    });
    $('.Sunday').click(() => {
        showActivitiesOnDate($('.Sunday').attr('dia'))
    });
}
